...
- [!] **E11-S01-T03 — Run the bats suite under bash ≥ 4 on macOS CI** *(In progress 2026-08-23: claimed by Claude session on branch `e11-s01-t03-macos-bash4` (main checkout). Filed by E03-S02-T05, which exposed it: with the macOS vitest step green for the first time, the job reached the bats step and failed there.)*
...
- [!] **E11-S01-T04 — Bats scratch paths must be physical, so macOS's `/var` symlink stops failing path assertions** *(In progress 2026-08-23: claimed by Claude session on branch `e11-s01-t03-macos-bash4`. Filed by E11-S01-T03, which exposed it: with bash ≥ 4 in place the macOS suite ran to completion for the first time and 9 checkout/multi-checkout tests failed on path equality.)*
