...
- [!] **E08-S02-T03 — `HelmDeploy@0` / `HelmInstaller@1` / `KubectlInstaller@0` / `Kubernetes@1` / `KubernetesManifest@1`** *(**All five run under real-task mode as of 2026-09-04 and all three Done items are met for the non-Azure arms — bats/vitest arg-and-connection assertions, live parity on a kind cluster, and the deltas in the warnings list. `[!]` stands for one thing only: the AKS/ARM arm of all three connection-consuming tasks is unreachable without an Azure service connection**, the same outward-facing write into the owner's Entra tenant that blocks E08-S01-T02 and E08-S02-T01. **Running them found three defects that reading them could not.** (1) Real-task mode was dropping every input the step did not write, so `task.json` **declared defaults never reached any task** (C-E08-073) — an E07 defect; `resolveTaskInputs` was built for it in E07-S01-T02 and called by nothing, and `Kubernetes@1` died on an undefined kubectl path because `versionOrLocation` never arrived. (2) `System.HostType` is dereferenced **at module load**, unguarded, by a helper all three tasks import (C-E08-072). (3) `Agent.ToolsDirectory` was set nowhere, and tool-lib throws before doing anything else without it (C-E08-068) — measured before *and* after against the real `KubectlInstaller@0`. **The collector was also emitting a warning that was actively false**: `connectionType: None` — the ambient arm — was told its empty `azureSubscriptionEndpoint` would fail with `LIB_InputRequired`, for an input `open()` returns before reading (C-E08-060). `CONNECTION_INPUT_RULES` encodes each task's own dispatch and **not `visibleRule`**, which `azureSubscriptionEndpointForACR` proves wrong on the first hard case (C-E08-063; decision 80). Helm parity is measured and negative by design: `helm version --client --short` is rejected by Helm 4, so `command: save` fails against a CLI that supports it (C-E08-069) — reported as a delta, not patched. Live transcript: `research/experiments/E08-kubernetes/real-task-run.md`.)*
