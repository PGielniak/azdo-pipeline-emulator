...
- [x] **E02-S05-T05 — `dependencies.<job>.result` / `stageDependencies.<stage>.<job>.result` in the shell backend** *(done 2026-08-28: both context arms compile to case-insensitive runtime result readers; executed nodes aggregate the existing five-state store, explicit skip markers preserve missing→Null, and corpus entry 03 now emits all entry points.)* *(filed 2026-08-25 by E05-S02-T02)*
  **Do:** `compileBash` implements only the `outputs[…]` arm of the `dependencies` context — `dependencies.web.result` throws `BashCompileError: context '…' is not readable by the shell backend`, so **entry-point emission fails outright** for any pipeline whose stage/job condition reads a dependency's result. Add the `result` arm (and the `stageDependencies` equivalent) as a read of the runtime results store, beside the existing `azdo_output` read.
  **Ground:** docs/02 §6 already lists `dependencies.<job>` (`result`, `outputs['step.var']`) as readable in a job/stage `condition:` (C-E02-080..091) — the gap is implementation, not grounding; the result **values** (`Succeeded`/`SucceededWithIssues`/`Failed`/`Skipped`/`Canceled`) and the store layout come from E06-S03's results store.
  **Done:** corpus entry `03-dependencies-and-conditions` emits its entry points (today it is the sole member of `ENTRYPOINTS_BLOCKED` in `packages/emit/test/readme.test.ts`, which asserts that list is exact); dual-backend conformance case for `.result` on both arms.
