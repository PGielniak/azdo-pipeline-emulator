...
- [!] **E01-S02-T02 — Strict validation of the expanded pipeline** *(blocked — **blocker narrowed 2026-08-22 by E12-S01-T02**: E00-S04 has landed (`expand()`/`expandCached()`/`resolveExpansion()` all return a `finalYaml`), so the missing half is no longer the client but the **wiring**: `convert`'s body still throws `NotImplementedError`, and hooking the validator to the expansion inside it waits on **E10-S02-T01**. Re-scoped from the original "two-pass validation" — the loose pass on the raw root is dropped, since raw-file validation is now the server's job. **Blocker narrowed again 2026-08-25 by E03-S04-T03:** the validator half now exists — `validateExpandedPipeline` reports against `pipeline.expanded.yml`, accepts the two output-only families an expansion legitimately carries (C-E03-258), and takes an `originAt` port so a diagnostic can point at the original source too. Nothing about validation is missing any more; the single remaining dependency is `convert`'s body, **E10-S02-T01**. **Blocker cleared 2026-08-26 by E10-S02-T01:** `convert` now calls `validateExpandedPipeline` on the service's `finalYaml` and reports against `pipeline.expanded.yml`, and a deliberately broken expansion fails with those pointers (`packages/cli/test/convert.test.ts`, "refuses an expansion that does not validate") while every corpus pipeline passes. Both Done criteria are reachable now; **closing this task is its own cycle** — E10-S02-T01 did not claim them.)*
