...
- [x] **E01-S02-T02 — Strict validation of the expanded pipeline** *(done 2026-08-26: the last dependency, `convert`'s body, landed with E10-S02-T01, and both Done criteria are now asserted **by name** rather than assumed to follow from the wiring existing (`packages/cli/test/convert.test.ts`, "E01-S02-T02 — strict validation of the expanded pipeline"). A deliberately broken expansion — a corpus `final.yml` mutated on the *service* side, since only the expansion can produce one — is rejected with `pipeline.expanded.yml:<line>:<col>` pointers in the message; and **every** committed corpus expansion passes clean. The validator itself is E03-S04-T03's `validateExpandedPipeline`, which is what made the second half reachable: an expansion is a different dialect from an authored document (C-E03-258), and the authored-document validator reports errors in nine of the ten corpus files. The task's blocker note travelled through three narrowings — E12-S01-T02 (2026-08-22), E03-S04-T03 (2026-08-25) and E10-S02-T01 (2026-08-26) — each recording what was actually left.)*
