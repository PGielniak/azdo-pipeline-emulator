...
- [x] **E00-S04-T02 — Expansion cache & lockfile entry** *(done 2026-08-22: `packages/fetch/src/expansion-cache.ts` — `cacheExpansion`/`readCachedExpansion` (`.cache/expansion/<requestHash>/final.yml` + `provenance.json`), `writeExpansionLockfileEntry` (merges an `expansion` field into `azdo-emu.lock.json` preserving other fields), `finalYamlHash` (sha256), and `expandCached` (fetch → cache+pin, or `--frozen` cache-only with `ExpansionCacheMissError`). Re-exported from index.ts. **Clarification of the Do's "(redacted)" wording:** the cached `final.yml` is stored **raw** because re-conversion needs the exact expansion and D8 guarantees the document carries no secret *values* (secrets are `.env`-owned) — `.cache/` is gitignored and the PAT is auth-only; the org/PAT scrubbing `redact()` stays for shared transcripts. docs/05 §4 updated with the layout + lockfile field. Tests: `packages/fetch/test/expansion-cache.test.ts` (8 tests), fetch suite 35 green, typecheck/eslint/prettier clean.)*
