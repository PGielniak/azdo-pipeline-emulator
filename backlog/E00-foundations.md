...
- [!] **E00-S03-T01 — Test-org runbook** *(offline part done 2026-07-30: `research/oracle-setup.md` written + fully grounded (C-E00-017..021 — route/api-version/body/response, scope vso.build="Build (read)", PAT Basic-auth/format/rotation). Remaining Done items need the user in a browser: (1) follow the runbook end-to-end, (2) org exists, (3) secrets stored (`.env.oracle` + GitHub repo secrets). Flip to `[x]` and fill the runbook's completion record when the Step-5 preview curl returns 200 + `finalYaml`.)*
