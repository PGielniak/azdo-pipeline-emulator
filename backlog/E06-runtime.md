...
- [x] **E06-S05-T02 — Checkout (self) modes & options**
  ...
  *Done 2026-08-22 — C-E06-097..111; `packages/runtime/test/core.bats` 12 new cases, 17 mutations confirmed red. The **Ground** field's "git flag mapping cited from git-scm.com" is necessary but not sufficient: git-scm says what `--depth`/`--tags`/`-ffdx` do, not which flags the agent emits, so the mapping is read from `GitCliManager`/`GitSourceProvider` at the already-pinned `42bde98b…` (BACKLOG §3 hierarchy) with git-scm cited for what those flags mean. Three of the six options have a default that lives in the pipeline-settings UI and no YAML file records, so the agent's empty-input defaults are used instead (decision 40). Two **Do**-field corrections: `clone` is the agent's `init`+`remote add`+`fetch`+`checkout` sequence over a `file://` remote, not a reference clone — git silently ignores `--depth` for a local path (C-E06-109) — and `copy` uses `tar`, not `rsync`, which is absent here. Headline finding: `fetchTags: false` does not stop tags being fetched, because the agent passes `--prune-tags` alongside `--no-tags` (C-E06-111). Multi-checkout layout, `persistCredentials`, `fetchFilter` and the two `sparseCheckout*` inputs are out of scope and reported rather than dropped (C-E06-110).*
