...
- [x] **E06-S04-T04 — Artifact/attachment commands (`artifact.upload`, `task.uploadfile`, `build.updatebuildnumber`, `build.addbuildtag`)**
  ...
  *Done 2026-08-21 — C-E06-069..083; `packages/runtime/test/core.bats` 12 new cases. Implemented with the three commands the named four cannot be split from: `task.addattachment`/`task.uploadsummary` are literally the same agent helper as `task.uploadfile`, and `build.uploadlog`/`build.uploadsummary` are one case arm each on top of it. `artifact.associate` is accepted-and-recorded rather than left unknown. Two docs/04 §6 rows were corrected rather than implemented (docs/06 §5 decision 37): there is no `task.uploadartifact` command, and `build.uploadlog` is a real attachment command, not "ignored with a debug note". `.artifacts/` is keyed by artifact **name** with the container folder as metadata. No hosted run: no oracle credentials in this environment.*

- [x] **E06-S04-T05 — Reconcile the two merged logging-command suites**
  **Do:** `packages/runtime/test/core.bats` carries two parallel test blocks for E06-S04-T01..T03 with **conflicting claim numbering** (one block reads C-E06-057 as `setsecret`, the other as `prependpath`) and three of them are red on `main`: `task.prependpath de-duplicates entries for subsequent tasks only` asserts two files in `path.d` where the trunk appends and de-duplicates newest-wins at the environment pass (same resulting `PATH`), and two rendering tests assert tag-preserving `\033[1;33m`-style ANSI that the trunk renderer does not emit (it strips the `##[tag]` prefix, drops `endgroup`, uses different codes, and needs `AZDO_COLOR=always` off a TTY). `azdo_step_issue_count` also has two signatures across the blocks. Pick one implementation per behavior, delete the losing test, renumber the losing claim block, and record which side won and why — the precedent is `b03c1f3`'s E03-S01-T02 reconciliation.
  **Ground:** the existing C-E06-012/057/058 (`path.d` newest-wins) and C-E06-066 (formatting commands are message-only; ANSI choices are explicitly *local*, so the renderer's exact codes are a decision to record in docs/06 §5, not a parity claim to measure).
  **Done:** `bats test/core.bats` fully green; one claim block per behavior; docs/06 §5 decision recording the reconciliation.
  *Done 2026-08-21 — the trunk block won both reds and the deleted block's claim numbering went with it (docs/06 §5 decision 38). Only the tests were ever duplicated: `lib/core.sh` has one definition per function, one renderer and one dispatch arm per command, so the five deleted cases were orphaned tests for an implementation that never landed. Two assertions were ported rather than dropped (`task.complete result=SucceededWithIssues` alone at exit 0; execution continuing past the complete line), claim-citation coverage was diffed mechanically rather than by eye, and C-E06-067 gained the test citation it had always lacked. Nothing to renumber in `research/`: it carries exactly one entry per ID and already used the trunk numbering. `bats test/core.bats` 67/67 green.*
...
- [x] **E06-S05-T02 — Checkout (self) modes & options**
  ...
  *Done 2026-08-22 — C-E06-097..113; `packages/runtime/test/core.bats` 14 new cases, 23 mutations confirmed red. The **Ground** field's "git flag mapping cited from git-scm.com" is necessary but not sufficient: git-scm says what `--depth`/`--tags`/`-ffdx` do, not which flags the agent emits, so the mapping is read from `GitCliManager`/`GitSourceProvider` at the already-pinned `42bde98b…` (BACKLOG §3 hierarchy) with git-scm cited for what those flags mean. Three of the six options have a default that lives in the pipeline-settings UI and no YAML file records, so the agent's empty-input defaults are used instead (decision 40). Two **Do**-field corrections: `clone` is the agent's `init`+`remote add`+`fetch`+`checkout` sequence over a `file://` remote, not a reference clone — git silently ignores `--depth` for a local path (C-E06-109) — and `copy` uses `tar`, not `rsync`, which is absent here. Headline finding: `fetchTags: false` does not stop tags being fetched, because the agent passes `--prune-tags` alongside `--no-tags` (C-E06-111). `submodules` needed a fix, not just a test: git has refused the `file` transport for submodule clones since CVE-2022-39253, so the option failed outright against the `file://` origin until `-c protocol.file.allow=always` was added — a deliberate local widening, decision 40f, C-E06-112. Multi-checkout layout, `persistCredentials`, `fetchFilter` and the two `sparseCheckout*` inputs are out of scope and reported rather than dropped (C-E06-110); a PR-shaped `Build.SourceBranch` override labels the run without reproducing the merge ref (C-E06-113).*
- [x] **E06-S05-T03 — Multi-checkout layout**
  ...
  *Done 2026-08-22 — C-E06-114..121; `packages/runtime/test/core.bats` 11 new cases, 19 mutations confirmed red — the last of which replaces `AZDO_HAS_MULTIPLE_CHECKOUTS` with a runtime that counts the checkouts it has already performed, and is caught by a `self`-last ordering case. The layout rule itself is on the doc page and was quoted; the **`Build.Repository.LocalPath` semantics** the **Do** field asks for are not, and they are the headline finding: under multi-checkout `self`'s files go to `s/<repoName>` while `Build.Repository.LocalPath` **and** `Build.SourcesDirectory` both stay at `s` (C-E06-116/117) — deliberate agent backward-compatibility that the obvious implementation (seed from the resolved target) gets wrong plausibly. Only a `path:` differing from the default moves `LocalPath`, and `path: s/<repoName>` spelled out is *not* different. The folder name is `RepositoryUtil.GetCloneDirectory` — git's clone-directory algorithm, not `basename` — ported with the agent's own 33-case L0 table as the conformance fixture (C-E06-118), plus non-parity guards refusing a name that reduces to `.`/`..`/`/`/nothing and the `x://` input that makes C# throw out of `Substring`. The multi-checkout signal is a **job** setting, so it arrives as `AZDO_HAS_MULTIPLE_CHECKOUTS` from the emitter rather than being counted (C-E06-115, decision 41): a counting runtime would treat the first of two checkouts as a single one. `workspaceRepo` is grounded and left to E05, because picking its winner needs the whole step list (C-E06-120); non-`self` checkouts seed no `Build.*` variables and never fall back to `AZDO_SELF_REPO` (C-E06-121). **No hosted run**: no oracle credentials in this environment. The **Ground** field's "verify one ambiguous combination (self+path+second repo) via real run" was satisfied from agent source instead — `RepositoryPlugin.GetRepoPath`, `BuildJobExtension.GetDefaultRepoLocalPathValue` and `BuildDirectoryManager.UpdateDirectory` each answer one half of it outright, and BACKLOG §3 ranks source above docs for runtime behavior (decision 41d).*
...
- [x] **E06-S06-T01 — Secret masking** *(done 2026-08-23 — **and every Done criterion was already met when the task was picked up, which is stated rather than papered over** (the E12-S02-T03 precedent). The E06-S04 logging-command lane built the whole of this task's **Do** on its way to `task.setvariable`/`task.setsecret`: `azdo_mask_stream`/`azdo__mask_line` are the streaming masker (`packages/runtime/lib/core.sh`), wired in `run_step` **before** the `tee` to the log file and before ANSI rendering, so console *and* log file carry masked bytes and the log keeps agent-shaped lines; `setsecret` additions register through the same path (C-E06-058). Masking replaces merged character **ranges**, not substrings one value at a time, so adjacent matches collapse to a single `***` as the agent does (C-E06-126). **The Do's open question is answered, and the answer is the opposite of a delta:** "handles values split across lines? (verify agent behavior — likely no; document delta)" → **C-E06-131, parity** — the agent registers no per-line fragment of a multiline secret (`AddUserSuppliedSecret` only *end*-trims CR/LF, C-E06-125), so a two-line secret survives both the agent and the emulator, and the Learn page's "We never mask substrings of secrets" is the rule it follows from. Recording that claim also fixed a real bug: the pre-existing masker read only the first line of each registered value, masking that line alone — *stricter* than the agent and inconsistent with the documented rule. Two genuine deltas are recorded beside it: the three global value encoders (C-E06-129, deliberately not implemented) and the URL-credential regex (C-E06-130, deferred to E08). Ground satisfied: the agent's masker is pinned at `microsoft/azure-pipelines-agent@42bde98bea7bb3b9e186d693e3b1554249e93a38` (`src/Agent.Worker/ExecutionContext.cs`) plus the set-secret-variables doc page. Done satisfied: bats `step stdout, stderr and the log file are all masked (C-E06-122)` covers both streams and the log, including the `failOnStderr` branch that routes stderr through a second `tee` before the masker. No code was written by this task; 231 Bats + 1,559 Vitest green.)*
