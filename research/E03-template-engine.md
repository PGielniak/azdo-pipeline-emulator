...
| `C-E03-120..139` | E03-S01-T02 conditional insertion chains | this file | 120–127 used |
...
## E03-S01-T02 — conditional insertion chains (`C-E03-120..127`)

Evidence: `research/experiments/E03-conditionals/` — **23 live preview probes** and 19 successful
input/`finalYaml` fixture pairs under `fixtures/oracle/directives/`, generated by
`pnpm template-conditionals-survey`. The official page documents the two insertion sites and one
chain spelling; the oracle settles chain state, recursion, truthiness, body shapes, and errors.

[C-E03-120] **Conditional directives run during template initialization and can insert into either
a sequence or a mapping; they are legal in a root pipeline as well as an included template.** The
official examples show `${{ if … }}` splicing steps into a sequence and environment entries into a
mapping, and explicitly allow the syntax outside templates.
...
[C-E03-121] **A conditional chain selects at most one branch: the first truthy `if`/`elseif`, or
`else` only when no condition matched; with no match and no `else`, it contributes nothing.** Once
a branch wins, later `elseif` conditions are not evaluated. Both sequence and mapping no-match
controls remove all directive bodies while retaining ordinary siblings.
  ...
  — `research/experiments/E03-conditionals/README.md` (first/elseif/else/no-match and
    short-circuit rows; live preview, checked 2026-08-18)

[C-E03-122] **The selected body is structurally spliced at the directive's position.** In sequence
position a Sequence body is flattened while a Mapping body becomes one item; in mapping position a
Mapping body's entries are merged. Surrounding siblings retain document order.
  — https://github.com/MicrosoftDocs/azure-devops-docs/blob/7d36475af0537d1e317e0a1cca2229c7eae20097/docs/pipelines/process/template-expressions.md#L180-L245
  — `research/experiments/E03-conditionals/README.md` (`sequence-*`, `mapping-*`, and
    `sequence-mapping-body`; live preview, checked 2026-08-18)

[C-E03-123] **Chain state belongs to the containing sequence/mapping and survives ordinary siblings.**
An `else` or `elseif` after an intervening step/key still belongs to the preceding `if`; the
ordinary sibling remains in output. A later `if` starts a new chain, so its following `else`
belongs to that new chain even when another unmatched `if` is immediately before it.
  — `research/experiments/E03-conditionals/README.md` (`interrupted-*` and
    `adjacent-independent-if`; live preview, checked 2026-08-18)

[C-E03-124] **A selected body is recursively expanded, with independent chain state in every
nested container.** A selected outer sequence branch can contain a nested sequence `elseif` chain
or a mapping `else` chain; only each nested winner survives.
  — `research/experiments/E03-conditionals/README.md` (nested-expansion row; live preview, checked
    2026-08-18)

[C-E03-125] **Directive conditions use expression truthiness, not a Boolean-only cast.** Null,
Boolean false, Number zero, and empty String are falsy; nonzero Numbers and nonempty Strings are
truthy; Version, Array, and Object are truthy, including empty Arrays/Objects. The collection rule
is observable because the documented conversion table does not define Array/Object→Boolean.
  — `research/experiments/E03-conditionals/README.md` (truthiness rows; live preview, checked
    2026-08-18)
  — `research/E02-expressions.md` C-E02-020 (primitive/Version Boolean conversions)

[C-E03-126] **`elseif`/`else` require a live chain in their containing container, and one `else`
terminates it.** An orphan `elseif`, an orphan `else`, and a second adjacent `else` are rejected
with `The expression directive '<keyword>' is not supported in this context`; in sequence position
the service also reports `Unexpected value '<raw directive>'`.
  — `research/experiments/E03-conditionals/README.md` (orphan/duplicate row; live preview, checked
    2026-08-18)

[C-E03-127] **A selected conditional in mapping position requires a Mapping body.** A Sequence body
is rejected `Expected a mapping`; by contrast, sequence position accepts either a Sequence to
flatten or a Mapping as one item (C-E03-122).
  — `research/experiments/E03-conditionals/mapping-sequence-body.md` (live preview, checked
    2026-08-18)
