# AGENTS.md — agent instructions for azdo-pipeline-emulator
...
2. **Decisions already made — do not relitigate** (record in `docs/06` §5; the six revisits of the simplification are entries 48–53, dated 2026-08-23): on-prem ADO Server out of scope; Windows host deferred; variable groups → `.env.example` names-only, never values; priority task set = the Azure/K8s deployment group (E08); every conversion emits a warnings/unsupported list and per-step fidelity labels — no coverage percentage, no gate (D10); expansion is delegated to the service (`preview` endpoint, see `docs/07-simplification-review.md`), with the retained local engine reachable only behind `--offline-expand` (D3); `script`/`bash`/`pwsh`/`powershell`/`checkout` are emitted as native bash and every other task runs via real-task mode or a stub — there is no per-task transpiler (D4); the **runtime** expression half (`$[ ]`, `condition:`, `$( )`, `dependencies.*`) is local and is *not* delegated (D6); one shared workspace per run, per-job isolation deferred (D8); host execution by default, container sandbox deferred (D9); converter is TypeScript/Node ≥ 22, output is dependency-free bash.
