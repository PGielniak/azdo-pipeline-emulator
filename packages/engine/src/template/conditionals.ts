 ...
 *
 * The service behavior is grounded by two independent live-preview surveys: 45 probes and 37
 * committed input/final-YAML pairs (`fixtures/oracle/directives/`; claims C-E03-120..137). Three
 * rules are easy to miss:
 *
 * - chain state belongs to the containing sequence/mapping, so an ordinary sibling does not end a
 *   chain (C-E03-128),
 * - the winning body is emitted at its directive's own document position (C-E03-128), and
 * - after one branch wins, later `elseif` expressions are not evaluated (C-E03-132).
 *
 ...
 * while leaving unselected bodies completely unevaluated (C-E03-126/133).
 */
