...
function quickStartSection(): string[] {
  return [
    ...
    '```',
    '',
    'If this pipeline consumes artifacts from another pipeline (`resources.pipelines`), they are',
    'pinned in `azdo-emu.lock.json` and served from `.cache/artifacts/`. `bash fetch-artifacts.sh`',
    're-downloads them — `--refresh` even when the cache is warm, `--latest` from each pipeline’s',
    'newest run instead of the pinned one (which fetches ahead without moving the pin).',
    '',
