...
13. **Expression errors: the service's sentence verbatim, our own coordinates (2026-08-12, E02-S01-T02).** 64 further live rejections (`research/experiments/E02-errors/`) settled how a rejection is *rendered*, and four of the answers are things we deliberately do not copy. (a) `(Line, Col)` locates the **host scalar**, never the offending token (C-E02-022) — we range the token so the code frame carets it, and keep the service's `Located at position N within expression` in the message, so the two remain reconcilable. (b) An expression embedded in a larger scalar is reported against a synthetic `format(…)` call the service compiles the scalar into (C-E02-026); that text is in no file the user owns, so we report their expression instead — and the finding is handed to E03-S01-T05 as evidence for interpolation. (c) Compile-time messages are truncated at 500 characters, mid-URL if need be (C-E02-024); we do not truncate, and the parity test truncates *ours* to prove equality. (d) The one runtime/compile-time **kind** swap (C-E02-015) is not reproduced: one grammar, one kind. Two findings changed the parser rather than the renderer: `!` is keyword-shaped, not a symbol (C-E02-018), and name resolution is **deferred** behind the syntax parse while arity is not (C-E02-020) — together these close the `! true` divergence E02-S01-T01 left open, and reclassify `+1`/`"double"`/`!true` as names that fail to resolve rather than lexical errors (C-E02-019). Updated: docs/02 §3 + §6; evidence `research/E02-expressions.md` C-E02-018..027.
