...
- **Match server quirks, not the YAML spec**, wherever they differ. Known items to encode as explicit behaviors with tests:
  - YAML anchors/aliases: Azure Pipelines does not support them → convert-time error with the same intent as the service (verify current server behavior at impl time; if the service has since added support, follow it).
  - Duplicate mapping keys: error (match server).
  - Only a single document per file; `---` separators rejected.
