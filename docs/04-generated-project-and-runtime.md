...
Stage result aggregates job results; `continueOnError: true` jobs degrade to `SucceededWithIssues`; the run exit code is non-zero when the pipeline result is `Failed` **or `Canceled`** *(corrected 2026-08-23, E06-S06-T02: this sentence read "iff the pipeline result is `Failed`", which enumerated the three job results and did not consider `Canceled` — a run the user interrupted must not report success to whatever invoked `run.sh`. docs/06 §5 decision 56)*. `azdo_run_result` folds every recorded step result worst-wins and `azdo_run_exit_code` maps it; `azdo_run_summary` prints the end-of-run table of §2. *(Extended 2026-09-21, E11-S04-T07: an **abandoned** stage or job — one whose `condition:` did not evaluate — also makes the run `Failed`, so the aggregate is no longer step results alone. Unlike the `Canceled` correction above this one is **measured, not reasoned**: a run whose only non-succeeded node was an abandoned stage came back `failed` from the service, with every failed stage removed from the probe so the result was attributable to nothing else — C-E12-054. docs/06 §5 decision 91.)*

**Variable seeding (added 2026-09-21, E11-S04-T03; docs/06 §5 decision 85).** A pipeline's authored `variables:` blocks are seeded into the store by the generated entry points, one scope per file: the root block by `run.sh`, the stage and job blocks by `run-job.sh`. The order is the documented precedence — job > stage > pipeline > queue time > Pipeline settings UI (C-E12-039) — and since PLAN D7 makes `.env` the stand-in for the last two rows, `run.sh` seeds **after** `azdo_env_load`, not before. It seeds before the run-number init, whose format may read a user-defined variable (C-E05-012). `run-job.sh` seeds stage then job into the job scope, after `azdo_var_scope_copy` and inside the same first-entry guard, so `--resume` retains a step's earlier `setvariable` writes. Values are stored raw and expanded on read, so one variable may refer to another. A `- group:` entry is skipped — it names a group, whose values arrive through `.env` and are never fetched — and nothing seeded here is marked secret, because a YAML block cannot declare one (secrets are marked from `manifest.json`'s `env[].secret`, C-E06-013). Every step condition is a `cond_step_<job>_<NNN>` function: `conditions.sh` is per stage and step numbers restart per job, so the job segment is what keeps two jobs' step 010 apart (C-E12-041).

**Stage and job conditions read the result store, not a step directory (added 2026-09-21, E11-S04-T04; C-E12-043/046).** `run-stage.sh` evaluates `cond_stage` once and then each `cond_job_<name>` in topological order, all in its own process — where `AZDO_RESULT_DIR` is deliberately unset, because that variable belongs to a *job*. So the status functions in those two slots compile to the graph-scope helpers (`azdo_status_{stage,job}_*`), which read `azdo_stage_result` / `azdo_job_result` over `state/results/<stage>/<job>` for the node's transitive `dependsOn` set — the same store the `eq(dependencies.<x>.result, …)` form already read, which is why that form kept working while the status functions did not. Job scope resolves its stage from the exported `AZDO_STAGE_ID` and fails loudly if it is unset, rather than silently ranging over nothing. `canceled()` reads `state/.run-canceled`, a marker nothing in a generated project writes today (docs/06 §5 decision 87).
...
```
File-per-value dodges quoting/newline pitfalls entirely. Variable keys are case-insensitive, matching the agent dictionary (C-E06-003). At first entry, each job copies the pipeline scope before adding its predefined/job-local values: root and `.env` values are therefore visible to every job, job writes shadow their inherited copies, and `setvariable` never leaks across jobs except through `outputs/` (C-E04-082/083, C-E05-017; docs/06 §5 decision 68). The copy includes the value and `.meta` sidecar, preserving secret/read-only/output flags without re-encoding them through the process environment.

The runner exports `AZDO_STATE_DIR` and the current `AZDO_VAR_SCOPE` before it sources the runtime.
`azdo_var <name> [scope]` reads a value (missing → empty); `azdo_var_set <name> <value> [secret] [output] [readonly] [scope]` writes it; and `azdo_var_meta <name> [scope]` prints the flags sidecar. `azdo_var_scope_copy <source> <target>` seeds a fresh, empty job scope and refuses a nonempty target; generated `run-job.sh` calls it exactly once, guarded by the job's predefined-directory initialization so `--resume` retains that job's prior state. An output write requires `AZDO_STEP_NAME` and `AZDO_OUTPUT_DIR`; it is stored as `<step>.<name>` in the current job and in `outputs/`, read cross-job by `azdo_output <stage> <job> <step.variable>`. Read-only overwrites emit an error and preserve the first value (C-E06-005/006). The logging-command handlers add two more runner-supplied directories to that contract, both required rather than defaulted: `AZDO_ARTIFACT_DIR` (the project's `.artifacts/`, used by `artifact.upload`/`artifact.associate`) and `AZDO_ATTACHMENT_DIR` (`<run>/logs/attachments/`, used by the attachment family). The emitter (E05) sets both alongside `AZDO_STATE_DIR`.
...
`publish`, `download` and `PublishPipelineArtifact@1`/`DownloadPipelineArtifact@2` are **runtime-performed**, like `checkout` (added 2026-09-21, E11-S04-T03): their packages declare an `AgentPlugin` handler and no Node one at all (C-E12-040), so real-task mode never had anything to exec. The emitter writes `azdo_artifact_publish`/`azdo_artifact_download` directly, and supplies the keyword's `$(Pipeline.Workspace)/<name>` target itself (C-E06-084) while leaving the task form to the task's own default (C-E06-085).
