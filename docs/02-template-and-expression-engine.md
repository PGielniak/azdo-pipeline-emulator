...
**Conditional insertion (grounded 2026-08-18, C-E03-120..127)** — mapping keys / sequence items `${{ if C }}:`, `${{ elseif C }}:`, `${{ else }}:` — selects the first truthy condition, otherwise `else`; no match contributes nothing. The winning body is structurally spliced at that directive's position (sequence bodies flatten, while mapping entries merge), then recursively expanded. Chain state belongs to the whole containing mapping/sequence: an ordinary sibling does **not** end it, while a new `if` starts a new chain and one `else` terminates it. Later `elseif` conditions are not evaluated after a winner. Conditions use expression truthiness: Null, false, zero, and empty String are false; Version and collections are true, including empty Array/Object. Orphan/duplicate clauses are errors, as is a non-Mapping winning body in mapping position. The 23-probe oracle matrix and all 19 successful input/`finalYaml` pairs live under `research/experiments/E03-conditionals/` and `fixtures/oracle/directives/`.
