...
Cross-job output reference `$[ dependencies.A.outputs['setSha.short'] ]` compiles to a read of `.work/state/outputs/<stage>/A/setSha.short` with Null→empty fallback. Dependency results compile to `azdo_job_result <stage> <job>` or `azdo_stage_result <stage>` over `state/results/<stage-id>/<job-reference>`; private skip markers distinguish a known skipped node from a missing member (C-E02-092..094; docs/06 §5 decision 70). String functions that are awkward in pure bash (`format`, `replace`, `trim`, …) compile to calls on `packages/runtime/lib/expr.sh` — still plain bash, sourced by the generated project.
